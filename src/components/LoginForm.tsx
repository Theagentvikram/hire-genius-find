  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        ...
        toast({
          title: "Login failed",
          description: "Invalid username or password. Try using 'recruiter' with password 'password123'.",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    >
      <Card className="w-full backdrop-blur-sm bg-white/90 shadow-xl border-t border-l border-white/20">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl text-center">Login to ResuMatch</CardTitle>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <Button 
                type="submit" 
                className="w-full bg-gradient-to-r from-brand-blue to-brand-purple hover:opacity-90 transition-all duration-300 gap-2"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
